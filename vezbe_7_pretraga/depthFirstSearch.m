%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%This code executes depth first search.  This simulation is meant to
    ...
    for x=-1:1
        for y=-1:1
            ...
            if (x*y==0)
                ...
                %If it doesn't fail any tests, add to beginning of fringe
                %set.  In depth first search nodes are removed from the
                %beginning of the fringe set, so to make things easy we add
                %new nodes to the beginning.
                if (failsTest<1)
                    if fringeCount<size(fringe,1)
                        fringe(size(fringe,1)+1,:)=[0,0,0];
                        prevNode=fringe(fringeCount+1,:);
                        for i=(fringeCount+2):(size(fringe,1))
                            nextNode=prevNode;
                            prevNode=fringe(i,:);
                            fringe(i,:)=nextNode;
                        end
                    end
                    fringe(fringeCount+1,:)=tempNode;
